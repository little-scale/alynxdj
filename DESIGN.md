...
TBS 0 advances once per note and preserves the row; 1–F advance every N
ticks and restart on note-on. Every mode wraps `0F→00`, while `H` defines a
shorter or non-zero loop point (D17). VOL can set the live level/peak during
