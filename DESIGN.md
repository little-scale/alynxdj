...
| Q5 | 🔶 **Software/emulator resolved; SD-cart incompatibility now evidenced.** `.lnx` byte 60 = 5 selects the 93C86; the release ROM and patched core power-cycle all 2048 bytes. A save copied from the reporting SD card on 2026-07-18 is only 128 bytes, has no `ALDJ` header, and decodes as FAT `.` / `..` directory entries rather than EEPROM data. M17's canonical EWEN therefore cannot be the whole issue: the cart firmware or card filesystem is not creating a 93C86 image. | M10/M17 | Record cart model + firmware; reformat/check the card, delete the bad `.sav`, then re-test SAVE → clean power-off → inspect the recreated file before changing the tracker protocol |
