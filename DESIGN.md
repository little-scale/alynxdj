...
- **IN24:** row-rate clock via the Pico bridge. The Pico counts standard USB
  MIDI Clock. `FA`/`FB` is followed immediately by the first `F8` row grant;
...
- `pico-midi-comlynx/` is the reference bridge. It enumerates as a USB-MIDI
  ...
  62.5 kbaud using PIO. It is send-only, has no heartbeat, and does not yet
  host a USB controller directly.
