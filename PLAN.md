...
| M17 | ✅ **HARDWARE LISTENING PASS FIXES** — NEW defaults TONE/ATK0/HOLD5/DCYA; empty CHAIN transpose displays 00 while retaining the FF-FF packed sentinel; TBS 0–F added in HOLD's high nibble with note/tick semantics and save v6; table VOL no longer revives decay; G is row-clocked (`G01` = one tap/row) and B is a signed current-taps offset. The PCM pump atomically snapshots/publishes its 16-bit ring pointers, and 93C86 EWEN now matches cc65's canonical command for strict SD-cart emulators. `make test-hardware` proves table clocks/lifetime, taps rates, and distinct sample lengths. | Silicon feedback closes the emulator blind spots |
...
**M0–M18 implemented as of 2026-07-18.** The tracker has all ten screens,
